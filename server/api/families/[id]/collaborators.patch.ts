...
export default defineEventHandler(async (event) => {
  ...
  const body = await readValidatedBody(event, (data) => UpdateRoleSchema.parse(data))
  ...
  if (!isOwner && !isAdmin) {
    throw createError({
      ...
      message: 'Anda tidak memiliki hak akses untuk mengelola peran di keluarga ini.'
    })
  }

  if (body.type === 'ACTIVE') {
    // Update active collaborator role
    const [updatedCollab] = await db
      .update(familyUserRoles)
      .set({ role: body.role })
      .where(
        and(
    ...
    return { success: true, collab: updatedCollab }
  } else {
